...
function newClient() {
    ...
    # Get client name from the environment or prompt
    if [[ -z "$CLIENT_NAME" ]]; then
        echo ""
        ...
        CLIENT_EXISTS=$(grep -c -E "^### Client ${CLIENT_NAME}\$" "/etc/wireguard/${SERVER_WG_NIC}.conf")
        if [[ ${CLIENT_EXISTS} != 0 ]]; then
            echo "Error: A client with the specified name already exists."
            exit 1
        fi
    fi

    # Automatically assign the next available IPv4 address
    ...
    # Automatically assign the next available IPv6 address
    for DOT_IP in {2..254}; do
        DOT_EXISTS=$(grep -c "${SERVER_WG_IPV6::-3}${DOT_IP}" "/etc/wireguard/${SERVER_WG_NIC}.conf")
        if [[ ${DOT_EXISTS} == '0' ]]; then
            CLIENT_WG_IPV6="${SERVER_WG_IPV6::-3}${DOT_IP}"
            break
        fi
    done
    ...
    if [[ -z "${CLIENT_WG_IPV6}" ]]; then
        echo ""
        echo "The subnet configured supports only 253 clients for IPv6."
        exit 1
    fi
